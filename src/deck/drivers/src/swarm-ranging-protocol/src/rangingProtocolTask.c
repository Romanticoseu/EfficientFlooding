...
extern uint8_t tx_red;
extern uint8_t tx_green;
extern uint8_t tx_blue;
static dwDevice_t* dwDevice;
static uint16_t devVelocityXId;
static uint16_t devVelocityYId;
static uint16_t devVelocityZId;
paramVarId_t Effect;
paramVarId_t SolidBlue;
paramVarId_t SolidRed;
extern uint16_t distancetest;
...
void RxCallback(dwDevice_t *dev) 
{
    ...
    DEBUG_PRINT_ALGO_DATA("TX MESSAGE SEQENCE: \n");
...
void TsTask(void *ptr)
{
    ...
    Effect=paramGetVarId("ring","effect");
    SolidBlue=paramGetVarId("ring","solidBlue");
    SolidRed=paramGetVarId("ring","solidRed");
...
void FTask(void *ptr)
{
    ...
    while (true)
    {
        xSemaphoreTake(globalSetLock, portMAX_DELAY);
        // critical section
        GenerateF(myAddress, &txFPacket, 3,tx_red,tx_green,tx_blue);
